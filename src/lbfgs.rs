...
impl Lbfgs {
  ...
  pub fn minimize(&self, x: &mut [f64], fx_gfx: &dyn Fn(&[f64]) -> (f64, Vec<f64>)) -> Status {
    ...
    for k in 1.. {
      ...
      let step_size = self.line_search.search(fx_gfx, x, z.slice());
      let dx = step_size * z;

      if !dx.slice().l2_norm().is_finite() {
        return Status::NaN;
      }

      gfx_prev = h;
      x_prev = x.to_vec().col_mat();
      x.clone_from_slice((&x_prev + dx).slice());
    }
  ...
  fn result_() -> Result<(), Box<dyn Error>> {
    ...
    let fx = |x: &[f64]| {
      let fx = 20.0
        - 20.0
          * (-0.2
            * (1.0 / x.len() as f64 * x.into_iter().map(|xi| xi.powi(2)).sum::<f64>()).sqrt())
          .exp()
        + E
        - (1.0 / x.len() as f64 * x.into_iter().map(|xi| (2.0 * PI * xi).cos()).sum::<f64>()).exp();
